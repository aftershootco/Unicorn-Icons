 ...
 * @param {string} outputDirectory
 */
export const appendFile = (content, fileName, outputDirectory) => fs.appendFileSync(path.join(outputDirectory, fileName), content, 'utf-8')
 ...
 * @param {string} outputDirectory
 */
export const writeFile = (content, fileName, outputDirectory) => fs.writeFileSync(path.join(outputDirectory, fileName), content, 'utf-8')
 ...
 */
export const readMetadata = (fileName, directory) => JSON.parse(fs.readFileSync(path.join(directory, fileName), 'utf-8'))
 ...
 * @param {string} directory
 */
export const readSvg = (fileName, directory) => fs.readFileSync(path.join(directory, fileName), 'utf-8')
 ...
 * @param {string} content
 */
export const writeSvgFile = (fileName, outputDirectory, content) => fs.writeFileSync(path.join(outputDirectory, fileName), content, 'utf-8')
...
export const hasDuplicatedChildren = (children) => {
	...
	return !hashedKeys.every((key, index) => index === hashedKeys.findIndex((childKey) => childKey === key))
}

import React from 'react'

const WarningImgMovedIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => {
	return (
		<svg {...props} width='12' height='12' viewBox='0 0 12 12' fill='none' xmlns='http://www.w3.org/2000/svg'>
			<g clipPath='url(#clip0_14076_44241)'>
				<path
					d='M4.35 1.5H7.65C7.8 1.5 7.9 1.55 8 1.65L10.35 4C10.45 4.1 10.5 4.2 10.5 4.35V7.65C10.5 7.8 10.45 7.9 10.35 8L8 10.35C7.9 10.45 7.8 10.5 7.65 10.5H4.35C4.2 10.5 4.1 10.45 4 10.35L1.65 8C1.55 7.9 1.5 7.8 1.5 7.65V4.35C1.5 4.2 1.55 4.1 1.65 4L4 1.65C4.1 1.55 4.2 1.5 4.35 1.5V1.5Z'
					stroke='#FADBDB'
					strokeLinecap='round'
					strokeLinejoin='round'
				/>
				<path d='M6 4V6' stroke='#FADBDB' strokeLinecap='round' strokeLinejoin='round' />
				<path d='M6 8H6.005' stroke='#FADBDB' strokeLinecap='round' strokeLinejoin='round' />
			</g>
			<defs>
				<clipPath id='clip0_14076_44241'>
					<rect width='12' height='12' fill='white' />
				</clipPath>
			</defs>
		</svg>
	)
}

export default WarningImgMovedIcon
